...
function ReefMode() {
  ...
  const refreshSnapshot = useCallback(async () => {
    try {
      ...
      let merged = (payload && payload.priorities && payload.priorities.length > 0)
        ? payload
        : { ...DEMO_DATA, ...(payload || {}) };

      // Pull research intel and attach into the UI contract
      try {
        const r = await jfetch(API.research);

        const synthesisInsights =
          (r && r.insights && r.insights.synthesis && r.insights.synthesis.data && r.insights.synthesis.data.insights) || [];
        const researchTrends =
          (r && r.insights && r.insights.trends && r.insights.trends.data) || null;
        const regulatoryHits =
          (r && r.insights && r.insights.regulatory && r.insights.regulatory.data && r.insights.regulatory.data.regulatory_hits) || [];

        // Top papers by relevance (server-provided)
        const topPapers = (r && r.top_papers) || [];
        ...
        merged = {
          ...
          researchTrends,
          regulatoryHits,
          researchHighlights: topPapers,
          systemStatus: {
            ...(merged.systemStatus || {}),
            researchAvailable: !!(r && r.available),
            researchPapers: (r && r.kb_stats && r.kb_stats.papers_total) || (merged.systemStatus && merged.systemStatus.researchPapers) || 0,
          },
        };
      } catch {
        // Research endpoint optional; keep baseline snapshot
      }
...
// Mount
const __reef_root = document.getElementById('root');
if (__reef_root) {
  const root = ReactDOM.createRoot(__reef_root);
  root.render(<ReefMode />);
}
